...
function printAgentEvent(
    ...
    // Check if it's a TaskArtifactUpdateEvent
        ...
        console.log(
            ...
            colorize(
                'yellow',
                'Received unknown event type in printAgentEvent:'
            ),
            event
        );
    }
...
function printMessageContent(message: Message) {
    message.parts.forEach((part: Part, index: number) => {
        ...
        if (part.kind === 'text') {
            // Check kind property
            console.log(
                `${partPrefix} ${colorize('green', '📝 Text:')}`,
                part.text
            );
            ...
            for await (const event of stream) {
                ...
                if (
                    ...
                    if (task.status.message) {
                        console.log(
                            colorize('gray', '   Task includes message:')
                        );
                    ...
                    console.log(
                        ...
                        colorize(
                            'yellow',
                            'Received unknown event structure from stream:'
                        ),
                        event
                    );
                }
